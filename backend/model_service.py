...
class OllamaModelProvider(ModelProvider):
    ...
    def __init__(self, model_name: str = "qwen2.5-coder:7b", base_url: str = "http://127.0.0.1:11434", timeout: int = 120):
    ...
    def analyze(self, language: str, code: str, error_message: str = "") -> dict[str, str]:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are CodeMate. Return ONLY a valid JSON object matching the requested schema. No conversational preamble or postscript."},
...
class DemoModelProvider(ModelProvider):
    ...
    def analyze(self, language: str, code: str, error_message: str = "") -> dict[str, str]:
        corrected_code = code
        result = {
            "bug_type": "No obvious issue",
            "detected_issue": "No obvious issue was found by the offline checks.",
            "explanation": "Demo Mode performs a small, deterministic syntax and runtime-risk scan. Configure Ollama or an API model for deep reasoning.",
            "suggested_fix": "Review the code with a configured LLM provider for deeper analysis.",
            "corrected_code": corrected_code,
            "reason": "The offline scanner did not identify a supported defect."
        }
        if language.lower() == "python":
            try:
                ...
                result.update(
                    ...
                    detected_issue=f"{exc.msg} on line {exc.lineno}.",
                    explanation="Python could not parse the source because its syntax is incomplete or invalid.",
                    suggested_fix="Add the missing syntax, such as the colon after a function definition.",
                    reason="The Python AST parser rejected the input.",
                    corrected_code=corrected_code
                )
            else:
                if "/0" in code.replace(" ", "") or "divide(10, 0)" in code:
                    ...
                    result.update(
                        ...
                        detected_issue="The code attempts to divide by zero.",
                        explanation="Division by zero raises ZeroDivisionError at runtime.",
                        suggested_fix="Validate the denominator before dividing.",
                        reason="The denominator is the literal value zero.",
                        corrected_code=corrected_code
                    )
                    ...
                    result.update(
                        ...
                        detected_issue="The comparison updates max_value when a number is smaller.",
                        explanation="This returns zero for all-positive inputs and does not compute the maximum.",
                        suggested_fix="Use `if n > max_value` and initialize from the input when negatives are valid.",
                        reason="Maximum tracking requires replacing the current value with larger numbers.",
                        corrected_code=corrected_code
                    )
            ...
            result.update(
                bug_type="Syntax Error",
                detected_issue="The return statement is missing a semicolon.",
                explanation="C requires a semicolon to terminate a return statement.",
                suggested_fix="Add the missing semicolon before the closing brace.",
                reason="The C statement is not terminated.",
                corrected_code=corrected_code
            )
            ...
            if corrected_code != code:
                result.update(
                    bug_type="Syntax Error",
                    detected_issue="The println statement is missing a semicolon.",
                    explanation="Java statements must end with semicolons.",
                    suggested_fix="Add the missing semicolon.",
                    reason="The Java statement is not terminated.",
                    corrected_code=corrected_code
                )
        if error_message:
...
def create_provider(config) -> ModelProvider:
    ...
    provider = str(get("MODEL_PROVIDER") or "demo").strip().lower()
    if provider == "ollama":
        base_url = get("OLLAMA_BASE_URL") or "http://127.0.0.1:11434"
        return OllamaModelProvider(get("MODEL_NAME") or "qwen2.5-coder:7b", base_url, get("MODEL_TIMEOUT") or 120)
    if provider == "api":
